...
SELECT 
'g49pi' as device, 
'Door1' as doorLocation, 
'lockStatus' as attribute, 
'unlock' as value, 
'desired' as desired_or_reported 
FROM '$aws/things/g49Trace/shadow/update/accepted' WHERE state.reported.authorised = true
...
def rest_handler(event, context):
    ...
    doorLocation = event.get("doorLocation")
    ...
    if value is None:
        ...
        set_state(device,doorLocation, desired_or_reported, attribute, value, timestamp)
        event["event"] = event["action"] + "Completed"
        return "OK"
...
def set_state(device2,doorLocation2, desired_or_reported, attribute2, value2, timestamp2):
