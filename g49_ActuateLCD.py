...
'''
SELECT 'g49pi' as device, 
'StudentsIDInArea' as attribute, 
state.reported.studentsIDInArea as value, 
'desired' as desired_or_reported 
FROM '$aws/things/g49Trace/shadow/update/accepted' WHERE state.reported.fire = 'yes'

Get or set the reported or desired state of a device and sensor (attribute).  The input looks like:
{
  "device": "g49pi",
  "attribute": "studentIDsInArea",
  "value": "state.reported.studentsIDInArea",
  "desired_or_reported": "desired"
}
Or for Slack commands:
{
  "channel_name": "g49",
  "user_name": "marklee",
  "text": "studentIDsInArea+9731 etc..."
}
...
def mark_handler(event, context):
    # Look for the device with the provided device ID and set its desired state.pa
    event["event"] = "ReceivedEvent"; print("JSON DUMP:", json.dumps(event))
    print ("MARKTEST-------------------------------:", json.dumps(event))
    # Deduce whether this is a get or set action.
    value = event.get("value")   #unlock in our case
    print ("VALUE-------------------------------:", value)
    ...
    event["event"] = event["action"]; print("JSON DUMP:",json.dumps(event))
...
def rest_handler(event, context):
    ...
    event["event"] = "ReceivedRESTEvent"; print("JSON DUMP:", json.dumps(event))
...
def get_state(device2, desired_or_reported):
    ...
    print("AWS RESPONSE:", json.dumps(response2))
...
def set_state(device2, desired_or_reported, attribute2, value2, timestamp2):
    print("SET STATE FUNCTION:----------",desired_or_reported )
    device3="g49Trace"
    ...
    # Post to AWS.
    payload2 = payload.copy(); payload2["event"] = "SendToAWS"; print("JSON DUMP:", json.dumps(payload2))
    iot_client = aws_session.client('iot-data')  # Create a client for AWS IoT Data API.
    ...
    if response.get('payload') is not None:
        response_payload = json.loads(response.get('payload').read().decode("utf-8"))  # Parse payload text to JSON.
        payload2["response_payload"] = response_payload; payload2["event"] = "GotAWSResponsePayload"; print("JSON DUMP:", json.dumps(payload2))
